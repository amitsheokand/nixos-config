...
pkgs.symlinkJoin {
  ...
  postBuild = ''
    rm $out/bin/rustdesk
    makeWrapper ${appimage}/bin/rustdesk $out/bin/rustdesk \
      --run 'case "''${1-}" in --check-hwcodec-config|--service) ;; *) (sleep 2; ${appimage}/bin/rustdesk --check-hwcodec-config >/dev/null 2>&1) & ;; esac'
  '';
}
