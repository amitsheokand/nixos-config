# Vendored from one-grep @ 1f215f9 (`nix/home-manager/one-grep.nix`), plus the
# `checkCommand` guard (mirrored upstream): skip MCP registration when the
# resolved command is missing, so hosts without the binary get no dead entries.
# OSS-safe upstream: no private hostnames, product names, or home paths.
# Re-vendor (do not fork) when the upstream module changes; if one-grep gains
# a public remote, prefer a flake input over this copy.
#
# Home Manager module for one-grep (OSS-safe).
#
# Import from this repo's shared wiring (`modules/shared/one-grep.nix`):
#   programs.one-grep = {
#     enable = true;
#     package = pkgs.one-grep;  # via overlays/one-grep.nix
...
in
{
  options.programs.one-grep = {
    ...
    package = mkOption {
      ...
      description = ''
        one-grep package. Supply via the overlay (`overlays/one-grep.nix`).
        Nullable so hosts can wire MCP with only `command` (e.g. a cargo-built
        binary on PATH).
      '';
    };
    ...
    assertions = [
      {
        ...
        message = ''
          programs.one-grep: set `package` (overlay `pkgs.one-grep`)
