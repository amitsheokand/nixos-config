...
let
  ...
  packages = [
    ...
    # ~/.pi/agent/mcp.json from programs.one-grep (stdio).
    ...
    # Muse Code Power via `muse login` — not Meta Model API PAYG.
    # Pin Spark 1.3 in pi-agents/muse-spark.md (bare muse-spark may be contributor).
  ...
  } // (import ./pi-chat-defaults.nix) // localSettings;
  ...
  overflowPick = import ./overflow-pick.nix { inherit pkgs lib; };

  # Declarative Pi extensions. Cursor is npm:pi-cursor-sdk; Muse Code is
  # npm:pi-muse-bridge (subscription). Do not register Meta Model API PAYG
  # (`pi-extensions/muse-spark.ts`) — that path bills per token.
  # OpenRouter overflow models are generated by overflow-assign (not pinned).
  # SoL-Pi is not installed. Action Fusion, local EPR, and obs-pack are
  # the one-offs; OCC stays /compact (no abort).
  piExtensions = [
    ./pi-extensions/action-fusion.ts
    ./pi-extensions/epr.ts
    ./pi-extensions/obs-pack.ts
  ];
  ...
  syncPiExtensions = ''
    rm -rf "$HOME/.pi/agent/extensions/happier-pi-tools-bridge"
    rm -f "$HOME/.pi/agent/extensions/"*-muse-spark.ts \
          "$HOME/.pi/agent/extensions/muse-spark.ts" \
          "$HOME/.pi/agent/extensions/"*-openrouter-ox-alpha.ts \
          "$HOME/.pi/agent/extensions/openrouter-ox-alpha.ts" \
          "$HOME/.local/bin/happier-desktop" \
          "$HOME/.local/bin/happier" \
          "$HOME/.local/bin/happier.real" \
          "$HOME/.local/share/applications/happier-desktop.desktop" \
          "$HOME/.local/share/applications/happier.desktop"
  ...
  npmPinnedVersion = src:
    let
      s = lib.removePrefix "npm:" src;
      parts = lib.splitString "@" s;
    in
      if lib.hasPrefix "@" s then
        if builtins.length parts >= 3 then lib.elemAt parts 2 else ""
      else
        if builtins.length parts >= 2 then lib.elemAt parts 1 else "";

  ensurePackage = src:
    let
      dir = npmDirName src;
      want = npmPinnedVersion src;
    in ''
      pkgjson="$HOME/.pi/agent/npm/node_modules/${dir}/package.json"
      have=""
      if [[ -f "$pkgjson" ]]; then
        have="$(${pkgs.jq}/bin/jq -r .version "$pkgjson" 2>/dev/null || true)"
      fi
      if [[ ! -d "$HOME/.pi/agent/npm/node_modules/${dir}" ]] || { [[ -n "${want}" ]] && [[ "$have" != "${want}" ]]; }; then
...
in
{
  home.packages = [ nodejs ] ++ (overflowPick.home.packages or []);
  systemdUserServices = overflowPick.systemdUserServices or {};
  systemdUserTimers = overflowPick.systemdUserTimers or {};
  launchdAgents = overflowPick.launchdAgents or {};
  ...
  sessionVariables = {
    ...
    # Herdr pane start: Composer 2.5 high, not fast, not MiniCPM5 longctx.
    HERDR_PI_MODEL = "cursor/composer-2-5:slow";
    HERDR_PI_THINKING = "high";
  };
  ...
  activation =
    ...
    // {
      ...
      syncMuseSparkAgent = lib.hm.dag.entryAfter [ "syncPiPackages" ] ''
        mkdir -p "$HOME/.pi/agent/agents"
        dest="$HOME/.pi/agent/agents/muse-spark.md"
        if [[ ! -f "$dest" ]] || grep -q 'managed-by: pi-muse-bridge' "$dest"; then
          install -m 0644 ${./pi-agents/muse-spark.md} "$dest"
        fi
      '';

      syncPiStack = lib.hm.dag.entryAfter [ "syncPiSettings" ] ''
        mkdir -p "$HOME/.pi/agent"
      ...
      syncHermesMemory = lib.hm.dag.entryAfter [ "syncPiSettings" ] ''
        ...
        install -m 0600 ${standingInstructions} "$HOME/.pi/agent/pi-hermes-memory/STANDING.md"
      '';
