...
let
  ...
  # Declarative Pi extensions. Do not register Meta Model API PAYG
  # (`pi-extensions/muse-spark.ts`) — that path bills per token.
  # Paid Muse/Cursor are native Herdr CLIs, not Pi npm bridges.
...
in
{
  ...
  sessionVariables = {
    ...
    # Herdr pane start: OpenCode Zen Nemotron 3 Ultra free, high thinking.
    HERDR_PI_MODEL = "opencode/nemotron-3-ultra-free";
  ...
  activation =
    ...
    // {
      ...
      syncPiPackages = lib.hm.dag.entryAfter [ "syncPiSettings" ] ''
        ...
        rm -rf "$HOME/.pi/agent/npm/node_modules/pi-muse-bridge" \
               "$HOME/.pi/agent/npm/node_modules/pi-cursor-sdk"
        ${lib.concatMapStrings ensurePackage packages}
      '';

      purgePaidPiBridges = lib.hm.dag.entryAfter [ "syncPiPackages" ] ''
        rm -f "$HOME/.pi/agent/agents/muse-spark.md"
      '';
